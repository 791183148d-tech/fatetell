...
def _resolve_gender_code(gender):
    """返回性别代码: 0=男, 1=女"""
...
def calc_bazi(year, month, day, hour, minute=0, gender="male"):
    ...
    gender_code = _resolve_gender_code(gender)
    ...
    liu_nian = _calc_liu_nian(da_yun_list, current_year, year, day_master["gan"], gender_code)
...
def _analyze_wuxing(count, dm_wx):
    ...
    return {
        ...
        "balance_advice": _get_balance_advice(strongest, weakest, dm_wx),
    }


def _get_balance_advice(strongest, weakest, dm_wx):
    """根据五行给出建议"""
    advice = {
...
def _calc_liu_nian(da_yun_cycles, current_year, birth_year, day_gan, gender_code):
    """计算当前流年 — 根据实际年龄确定当前大运。"""
    gan_zhi_list = ["甲子","乙丑","丙寅","丁卯","戊辰","己巳","庚午","辛未","壬申","癸酉",
...
if __name__ == "__main__":
    # CLI 测试: python bazi_engine.py 1990 5 15 12 male
    args = sys.argv[1:]
    if len(args) >= 4:
